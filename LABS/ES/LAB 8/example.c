...
int main(void){
    ...
    //Initilise the LCD
	lcd_init();
    //DDRAM storage of first line starts at 0x00 and D7=1 (0x80) for setting cursor address hence (0x00+0x80=0x80)
	lcd_comdata(0x80, 0);
	delay_lcd(800);
	lcd_puts(&Msg1[0]);

    //DDRAM storage of second line starts at 0x40 and D7=1 (0x80) for setting cursor address hence (0x40+0x80=0xC0)
	lcd_comdata(0xC0, 0);
	delay_lcd(800);
	lcd_puts(&Msg2[0]);
}
