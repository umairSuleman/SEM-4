...
void initPWM(void) {
    ...
    LPC_PWM1->PR = 0;                    //calculated earlier
    LPC_PWM1->MR0 = 30000;               //period=10ms
    LPC_PWM1->MCR = (1<<1);              //Reset PWM TC on PWM1MR0 match (bit 1=1 resets)
    LPC_PWM1->LER = 0xff;                //Update values in MR0 and MR1
    LPC_PWM1->TCR = 0x2;                 //Resets TC and PC (bit 1)
    LPC_PWM1->TCR = 0x9;                 //Enables PWM, TC, PC (bit 3:PWM bit 0: TC, PC)
}

void updatePulseWidth(unsigned int pulseWidth) {
    LPC_PWM1->MR4 = pulseWidth;          //Update MR4 with new Ton
    LPC_PWM1->LER = 0xff;                //Store into shadow register until it can be used in next time period
}
